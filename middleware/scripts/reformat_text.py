...
# Текст с двух сторон, а вокруг дефиса либо пробелы, либо с одной стороны запятая:
# слово - слово -> слово — слово
# слово, - слово -> слово, — слово
# слово -, слово -> слово —, слово
_TEXT_DASH_TEXT_RE = re.compile(r'(?<=\S)(?:,?\s+-\s+|\s+-\s+,?)(?=\S)')
...
def normalize_spaces(text: str) -> str:
    ...
    for line in lines:
        # 0) Дефис между фрагментами текста -> типографское тире
        # Срабатывает, только если по обе стороны есть текст.
        line = _TEXT_DASH_TEXT_RE.sub(lambda m: m.group(0).replace('-', '—'), line)

        # 0.1) Русские кавычки -> обычные двойные
        line = line.translate(_QUOTE_TRANS)
