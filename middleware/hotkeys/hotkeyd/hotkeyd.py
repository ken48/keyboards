...
# ---------- утилиты ----------
def event_mask(ev_type: int) -> int:
    return 1 << ev_type
...
# ---------- keycodes ----------
# Пытаемся брать системные константы (точнее и прозрачнее)
VK: Dict[str, int] = {}
...
# Фоллбек (если какие-то Quartz-константы недоступны) — минимум нужного:
VK.setdefault("space", 49);
VK.setdefault("spacebar", 49)
VK.setdefault("tab", 48);
VK.setdefault("return", 36);
VK.setdefault("esc", 53);
VK.setdefault("escape", 53)
VK.setdefault("left", 123);
VK.setdefault("right", 124);
VK.setdefault("down", 125);
VK.setdefault("up", 126)

# --- ANSI fallback (буквы/цифры) ---
ansi_fallback = {
...
class HotkeyDaemon:
    ...
    def _rebuild_cache(self):
        self._rulemap_exact.clear()
        for r in self.rules:
            self._rulemap_exact[(r["mods"], r["vk"])] = r
    ...
    def _find_rule(self, mods: Tuple[str, ...], vk: int) -> dict:
        for r in self.rules:
            if r["vk"] == vk and r["mods"] == mods:
                return r
        return None
    ...
    def _callback(self, proxy, etype, event, refcon):
        ...
        if etype == kCGEventKeyDown and not CGEventGetIntegerValueField(event, kCGKeyboardEventAutorepeat):
            ...
            if rule:
                if not self.busy.is_set():
                    threading.Thread(target=self._run_rule_blocking, args=(rule,), daemon=True).start()
                    if rule.get("swallow"):
                        return None
